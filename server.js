...
const cache = {};
const TTL = 5 * 60 * 1000;
function getCache(k) { const item = cache[k]; if(item && Date.now() - item.time < TTL) return item.data; return null; }
function setCache(k, data) { cache[k] = { time: Date.now(), data }; }
...
app.get("/api/:category", async (req,res)=>{
  ...
  try {
    ...
    const mapped = category==="news"?"general":category;
...
app.get("/api/summarize", async (req,res)=>{
  ...
  try {
    const prompt = `Summarize the following news article in 5-6 sentences:\n${content}`;
